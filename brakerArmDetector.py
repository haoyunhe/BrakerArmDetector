...
"""
  output:[dBraker01,dBraker02....]
         ...
         pt:(x,y)
"""
def brakerArmDetector(imgPath, firstRes):
    ...
    for res in firstRes:
        ...
        if res.name == "DoubleArmFold_IsolatingSwitch_Close":
            ...
            if res.miny < 0:
                res.miny = 0
            # cv2.rectangle(img, (res.minx, res.miny), (res.maxx, res.maxy), (0, 0, 255), 2)
            tempImg = img[res.miny:res.maxy, res.minx:res.maxx, :]
            # cv2.imwrite(resPath + fileName + "_" + str(ii) + ".jpg", tempImg)
            # ii = ii+1
            # print("tempImg:",tempImg.shape[:2])
            ...
            if len(result) != 0:
                # print(result[0])
                secondResDistance = result[0]
                for loc in result[1]:
                    pt1 = (loc[0][0] + res.minx, loc[0][1] + res.miny)
                    pt2 = (loc[1][0] + res.minx, loc[1][1] + res.miny)
                    tempLines = [pt1,pt2]
                    secondResLines.append(tempLines)
                    # cv2.line(img, pt1, pt2, (255, 0, 0), 2)
                tempRes.append(secondResDistance)
                tempRes.append(secondResLines)
            # else:
            #     print("no support braker...............")
        if len(tempRes) != 0:
            secondRes.append(tempRes)
        else:
            secondRes.append(tempRes)
    return secondRes
...
def filterPatternsWithArm(patterns, locs):
    ...
    flag02 = 0
    for i in range(length):
        if bins01[threshhold01] < patterns[i, 3] and bins01[threshhold01+1] > patterns[i, 3]:  ### 不同patch之间刀闸臂宽度相差大于5就丢弃
